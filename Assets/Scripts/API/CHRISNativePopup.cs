...
namespace TiltBrush
{
    public class CHRISNativePopup : PopUpWindow
    {
        ...
        public override void Init(GameObject parent, string text)
        {
            ...
            m_AutoPlaceButtons = Array.Empty<PopUpButton>(); m_OrderedPageButtons = Array.Empty<NavButton>();
            m_Persistent = true; m_BlockUndoRedo = true; m_TransitionDuration = 0.1f;
            m_ReticleBounds = new Vector3(3.8f, 4, -0.35f); m_PopUpForwardOffset = -0.4f;
            BuildView();
            base.Init(parent, "");
            if (m_Model != null)
            {
                m_Model.Changed += MarkDirty; m_Model.Opened(this);
        ...
        public void BuildView()
        {
            ...
            m_Status = m_Resources.Text(transform, "Status", new Vector3(0, 0.87f, -0.04f), new Vector2(3.5f, 0.55f), "Waiting for native gateway", 1.35f);
        ...
        {
            base.BaseUpdate();
            if (m_Dirty) { m_Dirty = false; Draw(); }
        }
        void Choice(int slot, string label, Action click, bool available = true)
        {
            var button = m_Choices[slot]; button.gameObject.SetActive(true); button.Label.text = label;
            button.Click = click; button.SetButtonAvailable(available && !m_Model.WaitingForRelease);
        ...
        void DrawDirect()
        {
            ...
            if (m_Model.ReviewedAction != null)
            {
                ...
                Choice(0, "Confirm change", m_Model.ConfirmDirect);
                Choice(1, "Cancel review", m_Model.Back); return;
            }
            if (m_Model.Category == "")
            {
                ...
                var categories = new[] { "Brush", "Color", "Size", "Panels", "Move", "Turn" };
                for (int i = 0; i < categories.Length; i++) { string category = categories[i]; Choice(i, category, () => m_Model.SetCategory(category), (bool)context["ready"]); }
                return;
            }
            var choices = CHRISPanel.Choices(m_Model.Category, context);
            int pages = Math.Max(1, (choices.Count + 3) / 4), page = Math.Min(m_Model.Page, pages - 1);
            m_Detail.text = m_Model.Category + " — page " + (page + 1) + " / " + pages + "\nSelect an option to review.";
            for (int i = 0; i < 4 && page * 4 + i < choices.Count; i++)
            { var option = choices[page * 4 + i]; Choice(i, option.Key, () => m_Model.Review(option.Value)); }
            Choice(4, "Previous page", () => m_Model.ChangePage(-1), page > 0);
            Choice(5, "Next page", () => m_Model.ChangePage(1), page + 1 < pages);
        }
        void DrawAssistance()
        {
            var client = m_Model.Assistance;
            m_Status.text = "Assistance\n" + (m_Model.WaitingForRelease ? m_Model.Notice : m_Model.AssistanceStatus());
            if (m_Model.Notice.StartsWith("STOP received")) m_Status.text = "STOP received locally.\n" + m_Model.AssistanceStatus();
            if (m_Model.ReviewedApproval != null)
            {
                m_Detail.text = CHRISPanel.Summary((JObject)m_Model.ReviewedApproval["action"], (JObject)client.Task?["context"]);
                bool current = (string)client.Task?["status"] == "awaiting_approval" && JToken.DeepEquals(m_Model.ReviewedApproval, client.Task["approval"]);
                var nativeContext = m_Model.Gateway.Capture();
                bool fresh = CHRISPanel.ApprovalCurrent(m_Model.ReviewedApproval, nativeContext, CHRISCommandGateway.Now);
                bool ready = (bool)nativeContext["ready"] && !(bool)nativeContext["stroke_active"];
                if (!fresh) m_Status.text = "Approval expired or state changed. Reject / cancel, then request a fresh proposal.";
                else if (!ready) m_Status.text = "Native host unavailable or drawing. Wait before approving.";
                Choice(0, "Approve change", () => m_Model.Decide(true), current && fresh && ready && !client.Busy && !client.CancelWanted);
        ...
        void OnDestroy()
        {
            if (m_Model != null) { m_Model.Changed -= MarkDirty; m_Model.Closed(this); }
